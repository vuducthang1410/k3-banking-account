...
public class CreditController {
    ...
    private final CreditHandler handler;
    private final WebUtils webUtils;
    private final LocaleUtils localeUtils;


    @GetMapping("")
    @Operation(summary = "Get Credit account list pagination")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> getPagination(
        ...
        PageDataDTO<CreditRp> pageData = service.findAll(page, pageSize);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Success", content =
                    {@Content(mediaType = "application/json", schema =
                    @Schema(implementation = CreditRp.class))}),
            @ApiResponse(responseCode = "400", description = "Bad Request", content =
                    {@Content(schema = @Schema(implementation = String.class))}),
            @ApiResponse(responseCode = "404", description = "Not Found", content =
                    {@Content(schema = @Schema(implementation = String.class))}),
    ...
    public ResponseEntity<?> getById(
        ...
        CreditRp data = service.findById(id);
        HttpStatus status = HttpStatus.OK;
        String msg = localeUtils.getLocaleMsg(MessageKeys.DATA_GET_SUCCESS, request);
    ...
    @PostMapping("")
    @Operation(summary = "Create Credit info")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> create(
            @RequestBody @Valid CreateCreditDTO data,
            WebRequest request
    ) {
        CreditRp createdData = handler.create(data);
